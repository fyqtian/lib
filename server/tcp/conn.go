...
func newConn(conn *net.TCPConn, srv *Server) *Conn {
	return &Conn{
		...
		packetSendChan:    make(chan Packet, srv.options.PacketSendChanLimit),
		packetReceiveChan: make(chan Packet, srv.options.PacketReceiveChanLimit),
	}
}
...
// Do it
func (c *Conn) Do() {
	if c.srv.callback.OnConnect(c) != nil {
		return
	}
...
func (c *Conn) handleLoop() {
	...
	for {
		...
		case p := <-c.packetReceiveChan:
			...
			if c.srv.callback.OnMessage(c, p) != nil {
				return
			}
		}
	}
